...
export default function InjuriesPage() {
  ...
  return (
    // Use a full-height flex container. Removing the stray `flex-1 flex-col`
    // class prevents leftover white space from appearing below the list and
    // allows the green background to extend all the way down to the bottom
    // navigation.
    <div className="flex flex-col h-full bg-[#C8E6C9]">
      ...
      <div className="bg-[#C8E6C9] px-6 pt-6 pb-4 flex flex-col items-center gap-4">
        ...
        <h1 className="text-2xl font-bold text-primary tracking-wide text-center">
          Lesões por Pressão
        </h1>
      ...
      <div className="flex-1 px-8 pb-8 overflow-y-auto flex flex-col gap-4 items-center">
        {topics.map((topic, index) => (
          <Link key={index} href={topic.path} className="w-full">
            <div className="w-full bg-[#F1F8E9] rounded-xl py-4 px-6 text-center shadow-sm cursor-pointer hover:bg-white transition-colors">
